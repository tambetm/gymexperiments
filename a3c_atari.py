...
def runner(shared_array, fifo):
    ...
    for episode in range(args.max_episodes):
        ...
        model.set_weights(pickle.loads(shared_array.raw))
...
def trainer(model, fifos, shared_array):
    step = 0
    while len(multiprocessing.active_children()) > 0:
        ...
        if maxlen > 0:
            ...
            shared_array.raw = pickle.dumps(model.get_weights(), pickle.HIGHEST_PROTOCOL)
...
if __name__ == '__main__':
    ...
    shared_array = Array('c', len(blob))
    shared_array.raw = blob
    env.close()
    ...
    for i in range(args.num_runners):
        ...
        fifos.append(fifo)
        process = Process(target=runner, args=(shared_array, fifo))
        process.start()

    trainer(model, fifos, shared_array)
